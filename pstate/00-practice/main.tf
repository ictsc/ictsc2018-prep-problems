...
data sakuracloud_archive "00-practice-archive" {
  name_selectors = ["2018-prep1-00-practice"]
}

## disks

resource sakuracloud_disk "00-practice-disk" {
  name              = "${var.PROBLEM}-practice-${var.TEAM_LOGIN_ID}"
  source_archive_id = "${data.sakuracloud_archive.00-practice-archive.id}"
  tags              = ["problem-${var.PROBLEM}", "${var.TEAM_LOGIN_ID}", "pstate_terraform"]
}

## servers
resource sakuracloud_server "00-practice-server" {
  name   = "${var.PROBLEM}-practice-${var.TEAM_LOGIN_ID}"
  core   = 1
  memory = 1
  disks  = ["${sakuracloud_disk.00-practice-disk.id}"]
